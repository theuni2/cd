...
import React, { useState, useCallback } from 'react'
import { motion, AnimatePresence } from 'framer-motion'
import { User, Mail, Phone, Briefcase, GraduationCap, Building, FileText, MessageSquare, ChevronRight, ChevronLeft, Check, Loader2, Square, CheckSquare } from 'lucide-react'
...
interface FormState {
  ...
  grade: string;
  schoolName: string;
  desiredOutcome: string[]; // Changed to Array for multiple selections
...
const useFormValidation = (initialState: FormState) => {
  ...
  const validateField = useCallback((name: keyof FormState, value: any) => {
    ...
    switch (name) {
      ...
      case 'schoolName':
        ...
        break
      case 'desiredOutcome':
        if (value.length === 0) error = "Select at least one outcome"
        break
      case 'fieldOfInterest':
        if (!value.trim()) error = "Please specify your field of interest"
        break
    }
    return error
  }, [])
  ...
  const handleChange = useCallback((event: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    setValues(prev => {
      ...
      setErrors(prevErrors => ({ ...prevErrors, [name]: validateField(name as keyof FormState, val) }));
      setTouched(prevTouched => ({ ...prevTouched, [name]: true }));
      return newValues;
    })
  }, [validateField])
  ...
  const toggleOutcome = (label: string) => {
    setValues(prev => {
      const currentOutcomes = prev.desiredOutcome.includes(label)
        ? prev.desiredOutcome.filter(item => item !== label)
        : [...prev.desiredOutcome, label];
      
      setErrors(prevErrors => ({ ...prevErrors, desiredOutcome: validateField('desiredOutcome', currentOutcomes) }));
      setTouched(prevTouched => ({ ...prevTouched, desiredOutcome: true }));
      return { ...prev, desiredOutcome: currentOutcomes };
    });
  }
  ...
  const validateForm = useCallback((currentStep: number) => {
    ...
    return currentFields.every(field => {
      ...
      return (Array.isArray(value) ? value.length > 0 : value !== '') && !hasError;
    }) && (!isLastStep || values.consent);
  }, [values, validateField, stepFields]);

  return { values, errors, touched, handleChange, handleBlur, toggleOutcome, validateForm }
}
...
export default function CareerForm() {
  ...
  const { values, errors, touched, handleChange, handleBlur, toggleOutcome, validateForm } = useFormValidation(initialState)
  ...
  const slideVariants = {
    enter: (direction: 'left' | 'right') => ({ x: direction === 'right' ? 500 : -500, opacity: 0 }),
    center: { x: 0, opacity: 1 },
    exit: (direction: 'left' | 'right') => ({ x: direction === 'right' ? -500 : 500, opacity: 0 })
  }
  ...
  const renderStep = () => {
    if (isSubmitted) {
      return (
        <div className="text-center">
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ duration: 0.5 }} className="inline-block p-4 bg-green-100 rounded-full mb-4">
            <Check className="w-12 h-12 text-green-500" />
          </motion.div>
          <h3 className="text-2xl font-semibold mb-4 text-primary">Thank You 🎉</h3>
          <p className="text-lg text-gray-600 mb-3">🎓 You’re on the <span className="font-semibold text-primary">Waitlist!</span></p>
          <p className="text-lg text-gray-600">Our team will reach out as soon as spots open for your 1:1 session.</p>
        </div>
      )
    }
      ...
      case 0:
        return (
          <div className="space-y-6">
            <h3 className="text-2xl font-semibold mb-6 text-primary">Basic Information</h3>
            <div className="space-y-4">
              <InputField icon={User} type="text" name="fullName" placeholder="Full Name" value={values.fullName} onChange={handleChange} onBlur={() => handleBlur('fullName')} error={errors.fullName} touched={touched.fullName} />
              <InputField icon={Mail} type="email" name="email" placeholder="Email Address" value={values.email} onChange={handleChange} onBlur={() => handleBlur('email')} error={errors.email} touched={touched.email} />
              <InputField icon={Phone} type="tel" name="phone" placeholder="Phone Number" value={values.phone} onChange={handleChange} onBlur={() => handleBlur('phone')} error={errors.phone} touched={touched.phone} />
              <div className="grid grid-cols-2 gap-4">
                {['Student', 'Parent'].map((type) => (
                  <label key={type} className={`flex items-center space-x-3 p-3 rounded-lg cursor-pointer border transition-all ${values.userType === type ? 'bg-blue-50 border-primary' : 'bg-gray-50 border-transparent'}`}>
                    <input type="radio" name="userType" value={type} checked={values.userType === type} onChange={handleChange} className="hidden" />
                    <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${values.userType === type ? 'border-primary' : 'border-gray-300'}`}>
                      {values.userType === type && <div className="w-2.5 h-2.5 bg-primary rounded-full" />}
                    </div>
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <InputField icon={Building} type="text" name="schoolName" placeholder="Name of School" value={values.schoolName} onChange={handleChange} onBlur={() => handleBlur('schoolName')} error={errors.schoolName} touched={touched.schoolName} />
            </div>
          </div>
        )
      case 2:
        return (
          <div className="space-y-6">
            <h3 className="text-2xl font-semibold mb-2 text-primary">Career Preferences</h3>
            <p className="text-sm text-gray-500 mb-4">Select all outcomes that interest you:</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {outcomeOptions.map((option) => {
                const isSelected = values.desiredOutcome.includes(option.label);
                return (
                  <motion.div
                    key={option.id}
                    whileHover={{ scale: 1.01 }}
                    whileTap={{ scale: 0.98 }}
                    onClick={() => toggleOutcome(option.label)}
                    className={`flex items-start space-x-3 p-4 rounded-xl cursor-pointer border-2 transition-all duration-200 ${
                      isSelected ? 'border-primary bg-blue-50 shadow-md' : 'border-gray-100 bg-gray-50 hover:border-gray-200'
                    }`}
                  >
                    <div className={`mt-0.5 ${isSelected ? 'text-primary' : 'text-gray-400'}`}>
            ...
            {touched.desiredOutcome && errors.desiredOutcome && <p className="text-red-500 text-sm mt-1">{errors.desiredOutcome}</p>}
            ...
            <div className="mt-6">
              ...
              <InputField 
                icon={GraduationCap} 
                name="fieldOfInterest" 
                placeholder="e.g., AI, Political Science, Business..." 
                value={values.fieldOfInterest} 
                onChange={handleChange} 
                onBlur={() => handleBlur('fieldOfInterest')} 
            ...
            <div className="space-y-4">
              <div className="relative">
                ...
                <textarea
                  ...
                  rows={3}
                  className="pl-10 w-full py-3 bg-gray-50 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-primary focus:bg-white transition-all"
                />
              </div>
              <div className="flex items-start space-x-3 bg-blue-50/50 p-4 rounded-xl border border-blue-100">
                ...
                <label htmlFor="consent" className="text-xs text-gray-600 leading-relaxed">
                  I consent to the collection and processing of my personal data by <span className="font-bold">Career Discovery</span> for the purpose of receiving guidance. I understand I can withdraw this at any time.
                </label>
              </div>
              {touched.consent && errors.consent && <p className="text-red-500 text-sm mt-1">{errors.consent}</p>}
            </div>
  ...
  return (
    <section className="py-20 relative overflow-hidden bg-gradient-to-br from-blue-50 via-blue-100 to-white min-h-screen">
      ...
      <div className="container mx-auto px-4 relative z-10">
        <h2 className="text-4xl font-bold text-center mb-12 text-[#011B3E]">Start Your Career Journey</h2>
        <div className="bg-white rounded-[32px] shadow-2xl p-8 md:p-12 max-w-4xl mx-auto border border-white/20 backdrop-blur-sm">
          {/* Progress Header */}
          <div className="mb-12">
            <div className="flex items-center justify-between relative">
              <div className="absolute top-5 left-0 w-full h-0.5 bg-gray-100 -z-10" />
              <div className="absolute top-5 left-0 h-0.5 bg-primary transition-all duration-500 -z-10" style={{ width: `${(currentStep / (steps.length - 1)) * 100}%` }} />
              {steps.map((step, index) => (
                <div key={index} className="flex flex-col items-center">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 ${
                    completedSteps.has(index) ? 'bg-green-500 text-white' : index === currentStep ? 'bg-primary text-white scale-110 shadow-lg' : 'bg-white border-2 border-gray-200 text-gray-400'
                  }`}>
                    {completedSteps.has(index) ? <Check size={20} /> : <step.icon size={18} />}
                  </div>
                  <span className={`mt-3 text-[10px] font-bold uppercase tracking-wider hidden md:block ${index <= currentStep ? 'text-primary' : 'text-gray-400'}`}>{step.title}</span>
                </div>
              ))}
            </div>
          </div>
            ...
            {!isSubmitted && (
              <div className="mt-12 flex justify-between items-center border-t border-gray-100 pt-8">
                <button
                  type="button"
                  onClick={prevStep}
                  className={`flex items-center px-8 py-3 font-semibold rounded-full transition-all ${currentStep === 0 ? 'opacity-0 pointer-events-none' : 'text-gray-500 hover:text-primary hover:bg-blue-50'}`}
                >
                ...
                {currentStep < steps.length - 1 ? (
                  <button
                    ...
                    onClick={nextStep}
                    disabled={!validateForm(currentStep)}
                    className={`flex items-center px-10 py-3 rounded-full font-bold shadow-lg transition-all ${
                      validateForm(currentStep) ? 'bg-primary text-white hover:bg-[#011B3E] hover:-translate-y-0.5' : 'bg-gray-200 text-gray-400 cursor-not-allowed'
                    }`}
                  >
                  ...
                  <button
                    type="submit"
                    disabled={!validateForm(currentStep) || !values.consent || isSubmitting}
                    className={`px-12 py-3 rounded-full font-bold shadow-xl transition-all flex items-center ${
                      validateForm(currentStep) && values.consent && !isSubmitting ? 'bg-green-600 text-white hover:bg-green-700 hover:-translate-y-0.5' : 'bg-gray-200 text-gray-400 cursor-not-allowed'
                    }`}
                  >
                    {isSubmitting ? <><Loader2 className="animate-spin mr-2" /> Sending...</> : 'Submit Application'}
                  </button>
                )}
              </div>
            )}
