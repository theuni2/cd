...
import React from 'react';
import { motion } from 'framer-motion';
import { UserCheck, BookOpen, BrainCircuit, Lightbulb, XCircle, Award } from 'lucide-react';
    ...
    <section className="py-24 relative overflow-hidden" style={{ backgroundColor: colors.bgDark }}>
      ...
      <div className="container mx-auto px-6 max-w-6xl relative z-10">
        ...
        <div className="text-center mb-16">
          ...
          >
            <h2 className="text-3xl md:text-5xl font-bold text-white mb-6">
              The <span style={{ color: colors.gold }}>PhD-Led</span> Advantage.
            </h2>
            <p className="max-w-2xl mx-auto text-lg" style={{ color: colors.slate }}>
              Winning global writing competitions requires rigorous academic depth. We pair you exclusively with <strong>PhD researchers and domain scholars</strong> to transform your ideas into world-class papers.
            </p>
          </motion.div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
          ...
          {/* Card 1: Academic Strategy */}
          <MentorCard 
            icon={<Award size={32} color={colors.gold} />}
            title="Scholarly Rigor & Ideation"
            subtitle="The Research Advisor"
            borderColor={colors.gold}
            points={[
              "Active PhD researchers guiding your core hypothesis",
              "Deep dive into peer-reviewed frameworks and advanced methodologies",
              "Elevates standard arguments into unique, sophisticated positions",
              "Ensures bulletproof logical flows capable of passing elite academic review"
            ]}
          />

          {/* Card 2: Competition Strategy */}
          <MentorCard 
            icon={<BookOpen size={32} color={colors.blue} />}
            title="Advanced Essay Crafting"
            subtitle="The Competition Strategist"
            borderColor={colors.blue}
            points={[
              "Mastery over rigorous global grading rubrics (e.g., John Locke, HIR)",
              "Refining style, tone, and technical vocabulary to collegiate levels",
              "1-on-1 editorial diagnostics on structure, syntax, and rhetoric",
              "Translating complex domain data into highly persuasive, narrative prose"
            ]}
          />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            >
                ...
                <ul className="space-y-4 mb-6">
                    <li className="flex items-start gap-3" style={{ color: '#e2e8f0' }}>
                        <span className="w-1.5 h-1.5 mt-2 rounded-full shrink-0" style={{ backgroundColor: colors.gold }} />
                        <span><strong>University-Level Mentorship:</strong> Learn exactly how Ivy League and Oxbridge professors evaluate critical insights.</span>
                    </li>
                    <li className="flex items-start gap-3" style={{ color: '#e2e8f0' }}>
                        <span className="w-1.5 h-1.5 mt-2 rounded-full shrink-0" style={{ backgroundColor: colors.gold }} />
                        <span><strong>Iterative Scholar Feedback:</strong> Direct feedback pipelines through every draft to establish unassailable factual accuracy.</span>
                    </li>
                    <li className="flex items-start gap-3" style={{ color: '#e2e8f0' }}>
                        <span className="w-1.5 h-1.5 mt-2 rounded-full shrink-0" style={{ backgroundColor: colors.gold }} />
                        <span><strong>The Ultimate Formulation:</strong> Merging high-level research exploration with razor-sharp competitive writing formats.</span>
                    </li>
                </ul>
            </motion.div>
            ...
            >
                <h3 className="text-xl font-bold text-white mb-6">The Scholarly Promise</h3>
                
                <div className="space-y-4">
                    <div className="flex items-center gap-3" style={{ color: colors.slate }}>
                        <XCircle size={20} color="#f87171" /> {/* Red-400 */}
                        <span>No Standard High School Templates</span>
                    </div>
                    <div className="flex items-center gap-3" style={{ color: colors.slate }}>
                        <XCircle size={20} color="#f87171" />
                        <span>No Superficial Surface Proofreading</span>
                    </div>
                    <div className="flex items-center gap-3" style={{ color: colors.slate }}>
                        <XCircle size={20} color="#f87171" />
                        <span>No AI-Generated Baselines</span>
                    </div>
                </div>
            </motion.div>
