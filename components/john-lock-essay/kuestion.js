            ...
            >
              ...
              <div className="p-6 flex items-center justify-between">
                <div className="flex items-center gap-4">
                   {/* Week Badge - Inside the card now */}
                   <div className={`px-3 py-1 rounded text-xs font-bold uppercase tracking-wider ${active === item.id ? 'bg-amber-500 text-slate-900' : 'bg-slate-800 text-slate-400'}`}>
                     {item.weeks}
                   </div>
                   <h3 className={`text-lg md:text-xl font-bold ${active === item.id ? 'text-white' : 'text-slate-300'}`}>
                     {item.phase}
                   </h3>
                </div>
                <ChevronDown className={`transition-transform duration-300 ${active === item.id ? 'rotate-180 text-amber-400' : 'text-slate-600'}`} />
              </div>
