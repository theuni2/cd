...
const ClimateProjectHero = () => {
  ...
  useEffect(() => {
    ...
    // --- Particle Logic (Client Side - Fireflies) ---
    const generatedParticles = [...Array(15)].map((_, i) => ({
      ...
      duration: 10 + Math.random() * 15,
      delay: Math.random() * 10,
      scale: Math.random() * 1 + 0.5, // 0.5 to 1.5 scale
      drift: (Math.random() - 0.5) * 200, // Horizontal drift in px
      size: i % 2 === 0 ? 4 : 8, // Pixel size
      color: i % 2 === 0 ? '#a3e635' : '#6ee7b7' // Lime or Emerald
    }));
  ...
  return (
    // SAFE MODE: Hardcoded Dark Green Background (Nature Theme)
    ...
    <section 
      ...
      style={{ backgroundColor: '#022c22' }} // Green-950
    >
      ...
      >
        ...
        >
          Turn Your Eco-Anxiety <br />
          <span 
            className="text-transparent bg-clip-text filter drop-shadow-[0_0_15px_rgba(163,230,53,0.4)]"
            style={{ 
              backgroundImage: 'linear-gradient(to right, #a3e635, #34d399)', // Lime to Emerald
              WebkitBackgroundClip: 'text'
            }}
        ...
        >
          ...
          <div 
            ...
            style={{ backgroundColor: 'rgba(2, 44, 34, 0.6)', border: '1px solid rgba(249, 115, 22, 0.4)' }}
          >
            <span className="flex items-center justify-center w-6 h-6 rounded-full text-sm" style={{ backgroundColor: 'rgba(249, 115, 22, 0.2)' }}>🔥</span>
            <span style={{ color: '#ffedd5' }}>Deadline: <span className="font-bold text-white">{dates.deadline}</span></span>
          </div>
          ...
          <div 
            className="flex items-center gap-3 px-5 py-3 rounded-full backdrop-blur-md shadow-lg transition-transform hover:scale-105 cursor-default"
            style={{ backgroundColor: 'rgba(2, 44, 34, 0.6)', border: '1px solid rgba(132, 204, 22, 0.4)' }}
          >
            <span className="flex items-center justify-center w-6 h-6 rounded-full text-sm" style={{ backgroundColor: 'rgba(132, 204, 22, 0.2)' }}>🌱</span>
            <span style={{ color: '#ecfccb' }}>Starts: <span className="font-bold text-white">{dates.start}</span></span>
          </div>
          ...
          <div 
            className="flex items-center gap-3 px-5 py-3 rounded-full backdrop-blur-md shadow-lg transition-transform hover:scale-105 cursor-default"
            style={{ backgroundColor: 'rgba(2, 44, 34, 0.6)', border: '1px solid rgba(20, 184, 166, 0.4)' }}
          >
            <span className="flex items-center justify-center w-6 h-6 rounded-full text-sm" style={{ backgroundColor: 'rgba(20, 184, 166, 0.2)' }}>🚀</span>
            <span style={{ color: '#ccfbf1' }}>Launch by: <span className="font-bold text-white">{dates.impact}</span></span>
          </div>
        ...
        >
          <a href='/#contact'>
            <button 
              className="px-8 py-4 text-white text-lg font-bold rounded-full transition-all duration-300 hover:-translate-y-1 shadow-[0_0_20px_rgba(132,204,22,0.3)] hover:shadow-[0_0_30px_rgba(132,204,22,0.6)]"
              style={{ 
                background: 'linear-gradient(to right, #65a30d, #047857)', // Lime-600 to Emerald-700
                border: 'none'
              }}
          ...
          <a href='#syllabus'>
            <button 
              className="px-8 py-4 text-lg font-semibold rounded-full transition-all duration-300 backdrop-blur-sm hover:-translate-y-1 hover:bg-lime-900/20"
              style={{ 
                backgroundColor: 'transparent',
                border: '1px solid rgba(190, 242, 100, 0.5)', // Visible Lime Border
                color: '#ecfccb' // Light Lime text
      ...
      {/* 3. Animation: "Fireflies" (Fixed Hydration) */}
      <motion.div 
        initial={{ opacity: 0 }}
        animate={{ opacity: 0.8 }}
        transition={{ delay: 1, duration: 2 }}
      ...
      >
        {particles.map((p) => (
          <motion.div
            key={p.id}
            initial={{ y: "110vh", x: `${p.initialX}vw`, opacity: 0 }}
            animate={{
              y: "-10vh",
              x: `${p.drift}px`,
              opacity: [0, 0.8, 0],
            ...
            className="absolute bottom-0 rounded-full shadow-lg"
            style={{ 
                width: p.size,
                height: p.size,
