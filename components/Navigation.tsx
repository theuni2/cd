...
const Navigation = () => {
  ...
  const navItems = [
    ...
    { href: '/essay_comp', label: 'Writing Programs' },
    { href: '/workshop', label: 'Career Labs' },
    { 
      href: '#', 
      label: 'Competitions', 
      subItems: [
          ...
          <div className="hidden md:flex items-center justify-end flex-1 space-x-8">
            {navItems.map((item) => {
              if (item.subItems) {
                return (
                  <div key={item.label} className="relative group">
                    <button className={`flex items-center gap-1 ${linkClass} py-2`}>
                      {item.label}
                      <ChevronDown className="w-4 h-4 transition-transform group-hover:rotate-180" />
                    </button>
                    {/* Dropdown Menu */}
          ...
          <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
            ...
            <SheetContent side="right" className="w-[300px] sm:w-[400px] pt-16">
              <SheetHeader>
                ...
                <SheetDescription className="text-left">Explore our programs and resources.</SheetDescription>
              </SheetHeader>
