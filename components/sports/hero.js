...
const SportsProjectHero = () => {
  ...
  useEffect(() => {
    ...
    // --- Particle Logic (Client Side - X's and O's) ---
    ...
    >
      ...
      >
        ...
        >
          Turn Your Passion for Sports <br />
          <span 
            className="text-transparent bg-clip-text filter drop-shadow-[0_0_15px_rgba(249,115,22,0.4)]"
            style={{ 
              backgroundImage: 'linear-gradient(to right, #f97316, #fbbf24)', // Orange to Amber
              WebkitBackgroundClip: 'text'
            }}
        ...
        >
          ...
          <div 
            className="flex items-center gap-3 px-5 py-3 rounded-full backdrop-blur-md shadow-lg transition-transform hover:scale-105 cursor-default"
            style={{ backgroundColor: 'rgba(15, 23, 42, 0.6)', border: '1px solid rgba(59, 130, 246, 0.4)' }}
          >
            <span className="flex items-center justify-center w-6 h-6 rounded-full text-sm" style={{ backgroundColor: 'rgba(59, 130, 246, 0.2)' }}>🏆</span>
            <span style={{ color: '#93c5fd' }}>Starts: <span className="font-bold text-white">{dates.start}</span></span>
          </div>
          ...
          <div 
            className="flex items-center gap-3 px-5 py-3 rounded-full backdrop-blur-md shadow-lg transition-transform hover:scale-105 cursor-default"
            style={{ backgroundColor: 'rgba(15, 23, 42, 0.6)', border: '1px solid rgba(249, 115, 22, 0.4)' }}
          >
            <span className="flex items-center justify-center w-6 h-6 rounded-full text-sm" style={{ backgroundColor: 'rgba(249, 115, 22, 0.2)' }}>👟</span>
            <span style={{ color: '#fdba74' }}>Launch by: <span className="font-bold text-white">{dates.outcome}</span></span>
          </div>
        ...
        >
          <a href='/#contact'>
            <button 
              className="px-8 py-4 text-white text-lg font-bold rounded-full transition-all duration-300 hover:-translate-y-1 shadow-[0_0_20px_rgba(234,88,12,0.3)] hover:shadow-[0_0_30px_rgba(234,88,12,0.6)]"
              style={{ 
                background: 'linear-gradient(to right, #ea580c, #d97706)', // Orange-600 to Amber-600
                border: 'none',
              }}
          ...
          <a href='#details'>
            <button 
              className="px-8 py-4 text-lg font-semibold rounded-full transition-all duration-300 backdrop-blur-sm hover:-translate-y-1 hover:bg-orange-900/20"
              style={{ 
                backgroundColor: 'transparent',
                border: '1px solid rgba(253, 186, 116, 0.5)', // Visible Orange Border
                color: '#fdba74' // Light Orange text
      ...
      {/* 3. Animation: "Playbook" Abstract Elements (Fixed Hydration) */}
      <motion.div 
        initial={{ opacity: 0 }}
        animate={{ opacity: 0.5 }}
        transition={{ delay: 1, duration: 2 }}
      ...
      >
        {particles.map((p) => (
          <motion.div
            key={p.id}
            initial={{ y: "110vh", x: `${p.initialX}vw`, opacity: 0, rotate: 0 }}
            animate={{
              y: "-10vh",
              x: `${p.drift}px`,
              opacity: [0, 0.4, 0],
            ...
            className="absolute bottom-0 font-bold text-white/5"
